...
TEST( GMPUtilTest, mpz64ToUint64Zero ) {
	try {
		...
		EXPECT_EQ( e.what(), "The number cannot negative: -1" );
	} catch ( ... ) {
		ADD_FAILURE();
	}
	...
	try {
		...
		EXPECT_EQ( e.what(), "The number cannot negative: -123456789" );
	} catch ( ... ) {
		ADD_FAILURE();
	}
}
...
TEST( GMPUtilTest, mpz64ToUint64OutOfRange) {
	try {
		...
		EXPECT_EQ( e.what(), "The number cannot be stored in a uint64_t: 18446744073709551616" );
	} catch ( ... ) {
		ADD_FAILURE();
	}
